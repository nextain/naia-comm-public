...
`naia-comm`는 Naia 커뮤니티의 참여 조사, 시작 미션, 주간 공유와 공개 운영 자료를 관리하는 프로젝트 저장소입니다. 공개 참여자는 자신의 fork에서 작은 결과를 만들고, 이슈와 pull request로 재현 가능한 증거를 제출합니다.
...
이 저장소의 협업 절차는 등록된 작업공간에서 Codex 또는 Claude가 코드 작업을 지원할 수 있다고 설명합니다. 원본 저장소의 스토어 검토 자료나 화면 예시에 OpenCode 표기가 있을 수 있지만, 그 자료는 이 공개 후보에 포함되지 않으며 현재 실행 가능한 공급자나 연결된 진입점을 증명하지 않습니다. Grok은 현재 이 트리에 별도 실행 진입점이 없으며 로컬 보조와 읽기 전용 canary(소규모 안전성 확인) 검토 범위로만 기록합니다. 실제 도구·모델·공급자 가용성은 실행 시점의 게이트웨이와 계정·환경 설정에 따르며, 이 저장소가 이를 보장하지 않습니다.
