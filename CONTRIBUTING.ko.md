...
커미터 사이의 저장소 간 조율은 maintainer 들의 비공개 조율 저장소에서 운영하며, 결과는 각 제품 저장소의 이슈로 공개됩니다. 아래 절차는 이 공개 저장소 자체에 기여하는 안내입니다.
...
비밀번호, 토큰, 개인 ID, 사설 호스트·IP, 고객 데이터, 운영 토폴로지를 commit·이슈·Discord에 남기지 않습니다. 이미지·NVA(아바타 설명 형식)·VRM·상표·책 자료의 원자산 조건은 [공개 자산 고지](NOTICE-PUBLIC-ASSETS.md)에서 확인합니다. 저장소 공개 전환은 전체 트리·생성물·도달 가능한 이력까지 검수하고 소유자가 정확한 커밋 SHA를 승인한 뒤에만 진행합니다. 이미 공개된 이 저장소의 fork·기여 제출은 위 온보딩 경로를 따릅니다. 비공개 자료의 신규 공개나 저장소 공개 전환은 별도 검수·승인 대상입니다.
